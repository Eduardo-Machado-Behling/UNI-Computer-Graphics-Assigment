...
export class Obj {
  ...
  CalculateWorldMatrix(parentMatrix) {
    ...
    this.worldMatrix = matrix4x4.Mult(
        this.worldMatrix,
        matrix4x4.Translation(
            this.position[0], this.position[1], this.position[2]));
    this.worldMatrix =
        matrix4x4.Mult(this.worldMatrix, matrix4x4.XRotation(this.rotation[0]));
    this.worldMatrix =
        matrix4x4.Mult(this.worldMatrix, matrix4x4.YRotation(this.rotation[1]));
    this.worldMatrix =
        matrix4x4.Mult(this.worldMatrix, matrix4x4.ZRotation(this.rotation[2]));
    this.worldMatrix = matrix4x4.Mult(
        this.worldMatrix,
        matrix4x4.Scaling(this.scale[0], this.scale[1], this.scale[2]));
    this.worldMatrix = matrix4x4.Mult(parentMatrix, this.worldMatrix);

    this.inverseTransposeWorldMatrix =
        matrix4x4.Transpose(matrix4x4.Inverse(this.worldMatrix));
  ...
  Draw(gl, viewMatrix) {
    ...
    gl.uniformMatrix4fv(
        this.program.u_matrix, true,
        matrix4x4.Mult(viewMatrix, this.worldMatrix));
    gl.uniformMatrix4fv(
        this.program.u_inverseTransposedMatrix, true,
        this.inverseTransposeWorldMatrix);
...
export class Cloud extends Obj {
  ...
  CreateVao(gl) {
    ...
    /* const indicesWireframe = new
     * Uint16Array(this.indexBufferWireframeCalculated.flat()); */
    ...
    /*
            ...
            gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, indicesWireframe ,
       gl.STATIC_DRAW); */
...
export class Sphere extends Obj {
  ...
  Lathe(gl) {
    ...
    for (let indexDivs = 0, angle = 0; indexDivs < this.divisions;
         angle = indexDivs * rateOfIncrease, indexDivs++) {
    ...
    for (let indexRow = 0; indexRow < this.divisions - 1; indexRow++) {
      for (let indexColumn = 1; indexColumn < this.pointsPerCurve - 1;
           indexColumn++) {
    ...
    for (let indexRow = 0; indexRow < this.divisions - 1; indexRow++) {
      for (let indexColumn = 0; indexColumn < columnScale - 1; indexColumn++) {
        indexBufferFaces.push([
          ...
          indexColumn + 1 +
              ((indexRow + 1) % (this.divisions - 1)) * columnScale
        ]);
        indexBufferFaces.push([
        ...
        /* indexBufferLines.push([
            indexColumn + indexRow * columnScale ,
            indexColumn + 1 + ( ( indexRow + 1 ) % ( this.divisions - 1 ) ) *
        columnScale
        ]); */
      }
    }
    ...
    for (let indexRow = 0; indexRow < this.divisions - 1; indexRow++) {
      indexBufferFaces.push([
        ...
        0 + indexRow * columnScale, startCapIndex
      ]);
      indexBufferFaces.push([
        endCapIndex, columnScale - 1 + indexRow * columnScale,
        columnScale - 1 + ((indexRow + 1) % (this.divisions - 1)) * columnScale
      ]);

      indexBufferLines.push([startCapIndex, 0 + indexRow * columnScale]);
      indexBufferLines.push([
        startCapIndex, 0 + ((indexRow + 1) % (this.divisions - 1)) * columnScale
      ]);
      indexBufferLines.push([
  ...
  AddNoise(gl) {
    ...
    for (let index = 0; index < this.vertexsBeforeNoise.length; index++) {
      sum = 0;
      originalVector = vector4.Create(
          this.vertexsBeforeNoise[index][0], this.vertexsBeforeNoise[index][1],
          this.vertexsBeforeNoise[index][2], 1);
      ...
      this.vertexs.push(vector4.ToVector3(vector4.Sum(
          originalVector,
          vector4.MultByEscalar(vector4.Normalize(originalVector), sum))));
    }
  ...
  CreateUvBuffer(gl) {
    ...
    for (let index = 0; index < this.perlinNoise.length; index++) {
      for (const element of this.uvBySum) {
        if (element.bottom < this.perlinNoise[index] &&
            element.top >= this.perlinNoise[index]) {
  ...
  CreateNormals(gl) {
    ...
    for (let index = 0; index < this.indexBufferFacesCalculated.length;
         index++) {
    ...
    for (let index = 0; index < normalsByVertex.length; index++) {
      this.normals.push(
          vector4.ToVector3(vector4.Normalize(normalsByVertex[index])));
    }
  ...
  CreateVao(gl) {
    ...
    const indicesWireframe =
        new Uint16Array(this.indexBufferWireframeCalculated.flat());
    const indicesFaces =
        new Uint16Array(this.indexBufferFacesCalculated.flat());
